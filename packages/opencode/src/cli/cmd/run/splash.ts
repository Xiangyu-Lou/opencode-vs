...
//
// Renders the full VsWorker entry logo and a compact [v] exit badge, plus
...
import { mark as brandMark } from "@/cli/logo"
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "VsWorker", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
