...
// etc.).
//
// The visual language mirrors the VsWorker app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `VsWorker is now connected to ${escapeHtml(provider)}.` : "VsWorker is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `VsWorker couldn't finish connecting to ${escapeHtml(provider)}.`
        : "VsWorker couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from VsWorker.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · VsWorker</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("VsWorker couldn't finish connecting to "+PROVIDER+"."):"VsWorker couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from VsWorker."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("VsWorker is now connected to "+PROVIDER+"."):"VsWorker is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// VsWorker wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 230 30" fill="none" aria-label="VsWorker" role="img">
        <path fill-rule="evenodd" clip-rule="evenodd" d="M0 0H7V13H0ZM21 0H28V13H21ZM60 0H67V30H60ZM82 0H89V30H82ZM153 0H160V30H153ZM32 7H55V13H32ZM93 7H120V13H93ZM124 7H148V13H124ZM170 7H178V13H170ZM182 7H205V13H182ZM209 7H230V13H209ZM7 12H10V30H7ZM18 12H21V30H18ZM71 12H78V26H71ZM165 12H170V26H165ZM3 13H7V21H3ZM21 13H25V21H21ZM32 13H39V21H32ZM93 13H100V30H93ZM113 13H120V30H113ZM124 13H131V30H124ZM170 13H174V17H170ZM182 13H189V30H182ZM198 13H205V21H198ZM209 13H216V30H209ZM160 15H165V22H160ZM39 16H55V21H39ZM189 16H198V21H189ZM67 20H71V30H67ZM78 20H82V30H78ZM170 20H174V30H170ZM6 21H7V30H6ZM10 21H18V30H10ZM21 21H22V30H21ZM48 21H55V30H48ZM32 24H48V30H32ZM100 24H113V30H100ZM174 24H178V30H174ZM189 24H205V30H189ZM71 26H72V30H71ZM76 26H78V30H76Z" fill="var(--oc-icon-base)" />
</svg>`
