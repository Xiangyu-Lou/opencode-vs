...
VsWorker is a fork of `anomalyco/opencode`. Everything the fork adds for bundling plugins, MCP servers, and skills
lives in `vsworker/`, except for nine small edits inside upstream files. Seven of them carry a `// vsworker-seam`
...
| `packages/opencode/test/preload.ts`           | sets the three `VSWORKER_DISABLE_BUNDLED_*` variables so upstream suites see the stock sets                                                      |
...
OpenCode installs plugins from npm at runtime into `~/.cache/opencode/packages/<spec>` and never re-resolves them.
