...
A skill is a **directory**, vendored under `vsworker/skills/<id>/`.
...
Hard constraints, from `resolveSkill` / `walk` in `vsworker/script/bundle.ts`:
...
# B. By hand: create vsworker/skills/<id>/, then add an entry to the skills array in bundle.jsonc
