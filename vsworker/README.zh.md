...
Skill 在本仓库里 vendored 在 `vsworker/skills/<id>/` 下，一个 `SKILL.md` 加上它需要的 `scripts/` /
`references/` 文件。每个文件都会被内联进构建，并在「带着已启用 skill 的构建」第一次启动时写到
`~/.cache/vsworker/vsworker/skills/<id>/`。它们需要磁盘上的真实目录，因为 `skill` 工具要列出同级文件，
斜杠命令也要解析相对路径。
...
编辑器和解释器的产物不会被打包：`__pycache__/`、`*.pyc`、`*.pyo`、`.DS_Store`、`Thumbs.db` 和 `.git/`
都会跳过 —— 它们每台机器都不一样，会让跑 `generate` 那台机器和 CI 算出的 skills 哈希对不上。

#### `env.json`

一个 skill 目录可以带一个 `env.json`。它里面的键值对会成为在该 skill 中运行的 bash 命令的环境变量。
...
把 `enabled` 挪进 `defaultEnabled`，并对明文密钥告警。`import skill` 把目录从
`~/.config/vsworker/skills/<name>` 拷进 `vsworker/skills/<name>`。两者都是追加进 `bundle.jsonc`，
不会破坏里面的注释。
